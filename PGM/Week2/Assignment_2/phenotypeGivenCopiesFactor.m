...
for k = 1:prod(phenotypeFactor.card)
    son = assignment(k, 1);
    motherCopy = assignment(k, 2);
    fatherCopy = assignment(k, 3);
    alleleNum = allelesToGenotypes(motherCopy, fatherCopy);
    if (son == 1)
        phenotypeFactor.val(k) = alphaList(alleleNum);
    else
        phenotypeFactor.val(k) = 1 - alphaList(alleleNum);
    endif
endfor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%