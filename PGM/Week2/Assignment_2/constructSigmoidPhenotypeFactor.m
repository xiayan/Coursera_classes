...
allelesNum = size(alleleWeights{1},2);
phenotypeFactor.card = [2, ones(1, 4) * allelesNum];
phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
total = prod(phenotypeFactor.card);
assignments = IndexToAssignment(1:total, phenotypeFactor.card);
for k = 1:total
    phenotype = assignments(k, 1);
    genotypes = assignments(k, 2:5);
    gene1 = genotypes([1,3]);
    gene2 = genotypes([2,4]);
    gene1Score = alleleWeights{1}(gene1(1)) + alleleWeights{1}(gene1(2));
    gene2Score = alleleWeights{2}(gene2(1)) + alleleWeights{2}(gene2(2));
    totalScore = gene1Score + gene2Score;
    if (phenotype == 1)
        phenotypeFactor.val(k) = computeSigmoid(totalScore);
    else
        phenotypeFactor.val(k) = 1 - computeSigmoid(totalScore);
    endif
endfor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%